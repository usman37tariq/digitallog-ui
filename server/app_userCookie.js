...
var cryptrSecret = "bT#JF(!3m4i-MXUL3q-48Ufd37SUZbHL]QR6bj@pK/Zfzv-ifi=V&UQ?FGZJJW-3)xruAQv}/6t,ZcAR)UytzU6FhhSuDGdD#$F$(]n2A!QgYt*RNv{:j]PkP;C@72?(e(/Z;}Pz]e@m$@yu4Tamf28dS(U:#9XC$c.QBdaZW_4;Akr(=]&-q#w:&z.Hb7d6bp@T&!2GJLFtdqe%{Jy}Y*D]7QZK{vJ4bUVLMzCCaiAfY]CEtivRTpe!n(iBvQx;";
...
var userCookieNameUser = '__DLN';//ifProd ? '__Secure-DLN' : '__Insecure-DLN';
var userCookieNamePass = '__DLP';//ifProd ? '__Secure-DLP' : '__Insecure-DLP';
 ...
 * START - Login Page
 */
app.get(['/login', '/login?credentials=invalid', '/login?error=request'], function (req, res, next) {
    ...
    if (cookieNameUser && cookieNamePass) {
        ...
        request(options, function (error, response, body) {
            ...
            if (response && response.statusCode === 200 && response.body && response.body.jwtToken) {
                ...
                console.log('|host>', req.get('host'), '|api>', req.session.paperless_api, '|ip>', _clientIp, '|u>', myName, '|p>', myPass, '|ua>', req.useragent.source);
...
app.post('/login', function (req, res, next) {
    ...
    if (req.body.user_id && req.body.user_password) {
        ...
        request(options, function (error, response, body) {
            ...
            if (response && response.statusCode === 200 && body) {
                // console.log('jwt', response.body.jwtToken);
                console.log('|host>', req.get('host'), '|api>', req.session.paperless_api, '|ip>', _clientIp, '|u>', req.body.user_id, '|p>', req.body.user_password, '|ua>', req.useragent.source);
